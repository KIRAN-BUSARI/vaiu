...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      `${request.nextUrl.origin}/sign-in?error=oauth_${error}`,
    );
  }
  ...
  if (!code) {
    ...
    const scope = "read:user user:email repo delete_repo";
  ...
  try {
    ...
    try {
      ...
      return NextResponse.redirect(
        `${request.nextUrl.origin}/sign-in?error=session_creation_failed`,
      );
    }
